...
- **Quality assessment is ultimately human.** There's no programmatic "plan quality score." The toolkit supports your judgment with structured comparison tables, dimension-by-dimension winners, and advocate debates that surface trade-offs — but the final call is yours. This is by design: automated plan evaluation is an unsolved problem, and LLM-as-judge has the same model biases the pipeline is trying to overcome.
